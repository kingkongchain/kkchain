...
func SetupGenesisBlock(db storage.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
...
func (g *Genesis) ToBlock(db storage.Database) *types.Block {
	if db == nil {
		db = memdb.New()
	}
...
func (g *Genesis) Commit(db storage.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db storage.Database) *types.Block {
...
func GenesisBlockForTesting(db storage.Database, addr common.Address, balance *big.Int) *types.Block {
