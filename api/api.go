...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.Receiver(); to != nil {
			return fmt.Sprintf("%s: %v wei + %v gas × %v wei", tx.Receiver().Hex(), tx.Value(), tx.GasLimit(), tx.GasPrice())
		}
		return fmt.Sprintf("contract creation: %v wei + %v gas × %v wei", tx.Value(), tx.GasLimit(), tx.GasPrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
//
// https://github.com/invin/kkchain/wiki/Management-APIs#personal_sign
...
//
// https://github.com/invin/kkchain/wiki/Management-APIs#personal_ecRecover
...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, vmCfg vm.Config, timeout time.Duration) ([]byte, uint64, bool, error) {
	defer func(start time.Time) {
		log.Debugf("Executing EVM call finished,runtime: %v", time.Since(start))
	}(time.Now())
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64) *RPCTransaction {
	var signer types.Signer = types.InitialSigner{}
	if tx.Protected() {
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, gasPrice *hexutil.Big, gasLimit *hexutil.Uint64) (common.Hash, error) {
	...
	for _, p := range pending {
		var signer types.Signer = types.InitialSigner{}
		if p.Protected() {
			signer = types.NewInitialSigner(p.ChainId())
		}
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", pow.SeedHash(number)), nil
}
...
func NewPublicNetAPI(net *p2p.Network, networkVersion uint64) *PublicNetAPI {
