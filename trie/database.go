...
type Database struct {
	diskdb storage.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb storage.Database) *Database {
	return &Database{
		diskdb:    diskdb,
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	nodes, storageSize, start := len(db.nodes), db.nodesSize, time.Now()
	...
	db.flushsize += storageSize - db.nodesSize
...
func (db *Database) commit(hash common.Hash, batch storage.Batch) error {
