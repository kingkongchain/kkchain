...
// the network.
func (f *Fetcher) Notify(peer string, hash common.Hash, number uint64, time time.Time, blockFetcher blockRequesterFn) error {
	block := &announce{
		hash:        hash,
		number:      number,
		time:        time,
		origin:      peer,
		fetchBlocks: blockFetcher,
	}
	select {
	case f.notify <- block:
		return nil
...
func (f *Fetcher) loop() {
	...
	for {
		...
		case filter := <-f.blockFilter:
			...
			blocks := []*types.Block{}
			for i := 0; i < len(task.blocks); i++ {
				// Match up a body to any possible completion request
				matched := false

				for hash, announce := range f.completing {
					if f.queued[hash] == nil {
						txnHash := task.blocks[i].TxRoot()
						...
						if txnHash == announce.header.TxRoot && announce.origin == task.peer {
							...
							if f.getBlock(hash) == nil {
								blocks = append(blocks, task.blocks[i])
							} else {
								f.forgetHash(hash)
							}
						}
				...
				if matched {
					task.blocks = append(task.blocks[:i], task.blocks[i+1:]...)
					i--
					continue
				}
			}
